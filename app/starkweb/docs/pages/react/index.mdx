...
For new projects, we recommend setting up your Starkweb application with the [`create-starkweb`](/cli) command line interface (CLI). This tool initializes a new Starkweb project with TypeScript and installs all required dependencies.
...
- [TanStack Query](https://tanstack.com/query/v5): This async state manager handles requests, caching, and more, making it a valuable tool when working with Wolf.
...
function App() {
  return (
    <StarkwebProvider config={config}> // [!code focus]
      {/** ... */} // [!code focus]
...
Inside the `WolfProvider`, wrap your app in a TanStack Query React Context Provider, e.g. `QueryClientProvider`, and pass a new `QueryClient` instance to the client property:
...
:::code-group
```tsx [profile.tsx]
import { useAccount, useBalance } from "sn-wolf"

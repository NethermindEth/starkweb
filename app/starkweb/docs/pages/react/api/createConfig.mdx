...
- See [Chains](/react/api/Chains) for more details about built-in chains and the `Chain` type.
...
[Connectors](/react/api/connectors) used by the `Config`.

```ts
import { createConfig, http } from "starkweb/react"
import { mainnet, sepolia } from "starkweb/react/chains"
import { injected } from "starkweb/react/connectors" // [!code focus]
...
- Enables discovery of injected providers via [EIP-6963](https://eips.ethereum.org/EIPS/eip-6963) and converting to [injected](/core/connectors/injected) connectors.
- Defaults to `true`.
...
- [`Storage`](/react/api/Storage) used by the config. Persists `Config`'s [`State`](#state-1) between sessions.
...
Mapping of [chain IDs](#chains) to [`Transport`](/react/transports). This mapping is used internally when creating chain-aware `Client` objects. See the [Transports docs](/react/transports) for more info.

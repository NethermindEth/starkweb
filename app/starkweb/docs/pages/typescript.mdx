...
- It is highly recommended that you lock your `sn-wolf` and `typescript` versions to specific patch releases and upgrade with the expectation that types may be fixed or upgraded between any release.
...
[Declaration merging](https://www.typescriptlang.org/docs/handbook/declaration-merging.html) allows you to **register** your `config` globally with TypeScript. The `Register` type enables Wolf to infer types in places that wouldn't normally have access to type info via React Context alone.
...
declare module "starkweb" { // [!code focus]
...
Wolf can infer types based on [ABIs](https://book.cairo-lang.org/ch15-01-contract-class-abi.html) and [SNIP-12](https://github.com/starknet-io/SNIPs/blob/main/SNIPS/snip-12.md) Typed Data definitions. This achieves full end-to-end type-safety from your contracts to your frontend and enlightened developer experience by autocompleting ABI item names, catching misspellings, inferring argument and return types (including overloads), and more.
...
:::tip
Unfortunately [TypeScript doesn't support importing JSON `as const` yet](https://github.com/microsoft/TypeScript/issues/32063). Check out the [Wolf CLI](/cli) to help with this! It can automatically fetch ABIs from Etherscan and other block explorers, resolve ABIs from your projects, generate React Hooks, and more.
:::

...
import type ControllerProvider from "@cartridge/controller"
  export type CatridgeParameters = {
  ...
  catridge.type = 'catridge' as const
  export function catridge(catridgeParameters: CatridgeParameters) {

    type Properties = {
      onConnect(connectInfo: ProviderConnectInfo): void
    }
    type StorageItem = { 'catridge.disconnected': true }
    type Provider = WalletProvider & ControllerProvider
    ...
    return createConnector<Provider, Properties, StorageItem>((config) => ({
      ...
      async setup() {
        ...
        controller = new Controller(catridgeParameters);
        (window as any).starknet_catridge = controller

        if (typeof window.starknet_catridge !== 'object') return
        const provider = await this.getProvider()
        await config.storage?.setItem('catridge.disconnected', true)
        if (provider && window.starknet_catridge?.isConnected) {
      ...
      async connect({ chainId, isReconnecting } = {}) {
        ...
        try {
          if (!accounts?.length) {
            const requestAccount = await provider.connect()
            accounts = [requestAccount?.address as Address]
          }
          ...
          await config.storage?.removeItem('catridge.disconnected')
      ...
      async getAccounts() {
        try {
          const provider = await this.getProvider()
          const accounts = [(await provider.connect())?.address]
          return accounts.map((x) => getStarknetAddress(x as string))
        } catch (error) {
      ...
      async getProvider() {
        ...
        const provider = (window as any).starknet_catridge
        if (!provider || typeof provider.request !== 'function') {
          throw new Error('Catridge provider not found')
        }
        return provider
      },
      async isAuthorized() {
        try {
          const isDisconnected =
            ...
            await config.storage?.getItem('catridge.disconnected')
      ...
      async switchChain({ chainId}) {
        ...
        try {
          await Promise.all([
            provider.switchStarknetChain(chainId),
      ...
      async onDisconnect(error) {
        ...
        try {
          if (typeof localStorage !== 'undefined') {
            localStorage.removeItem('Catridge_cached_address')
            localStorage.removeItem('Catridge_cached_chainId')
          }
        } catch (storageError) {

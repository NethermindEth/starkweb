...
const cli = cac('create-starkweb');

cli.usage(`${pc.green('<project-directory>')} [options]`)
    .option(
        '-t, --template [name]',
...
const defaultTargetDir = 'starkweb-project';
    ...
    try {
        result = await prompts(
            [
                ...
                {
                    type(_: unknown, { }: { overwrite?: boolean }) {
                ...
                {
                    type(_: unknown, { overwrite }: { overwrite?: boolean }) {
                        if (overwrite === false)
                            throw new Error(
                ...
                },
                {
                    type(framework: Framework) {
                        return framework?.variants?.length > 1
                            ? 'select'
                            : null;
                    },
                    name: 'variant',

...
export type GetStarknetIdUserDataParameters = Prettify<{
  /** Starknet ID, domain, or address to get the user data for. */
  idDomainOrAddr: string
  /** Field to retrieve from user data. */
  field: string
}>

export type GetStarknetIdUserDataReturnType = bigint

export type GetStarknetIdUserDataErrorType = Error

export async function getStarknetIdUserData<
  TChain extends Chain | undefined,
  TAccount extends Account | undefined,
>(
  client: Client<Transport, TChain, TAccount>,
  { idDomainOrAddr, field }: GetStarknetIdUserDataParameters,
  ...
  try {
    const result = await client.request({
      ...
      params: {
        request: {
          ...
          entry_point_selector: getSelectorFromName('get_user_data'),
    ...
    throw new Error(`Could not get user data from starknet id: ${error}`)
  }
}
